...
load("C:/Users/andre/Documents/HBGDki/Results/Risk_Factor_Results_wastinc6-24.Rdata")


#Drop trisls with only yearly measurements
for(i in 1:length(wastinc_624_unadj)){
  wastinc_624_unadj[[i]] <- wastinc_624_unadj[[i]] %>% filter(
...
REpooled_wastinc624_unadj <- NULL

setwd("C:/Users/andre/Documents/HBGDki/Figures/Risk Factor Analysis")

pdf("Unadjusted Risk Factor Plots Wasting CI 6-24.pdf", height=10, width=8)

for(i in 1:length(wastinc_624_unadj)){
  if(nrow(wastinc_624_unadj[[i]])>0 & 
     sum(is.na(wastinc_624_unadj[[i]]$RR),na.rm=T) != nrow(wastinc_624_unadj[[i]])
    ){
    
    if(length(unique(wastinc_624_unadj[[i]]$meanLevel))>13){
    plotlevels <- c("Q1","Q2","Q3","Q4")  
    wastinc_624_unadj[[i]]$meanLevel <- wastinc_624_unadj[[i]]$level <- rep(plotlevels, nrow(wastinc_624_unadj[[i]])/4)
    }else{
    plotlevels <- unique(wastinc_624_unadj[[i]]$meanLevel)  
    }
                 
  Anywast <- cleandf(wastinc_624_unadj[[i]], RF_levels=plotlevels)   #c("<=2700","2700-3000","3000-3400",">3400"))
  Anywast_plot <-RRplot_fun(Anywast, 
                               reflevel=wastinc_624_unadj[[i]]$level[1], 
                               title=paste0("Risk factor: ",wastinc_624_unadj[[i]]$variable[1],"\nCumulative incidence ratios:\nAny wasting from 6-24 months age"), 
                               units="",
                               levels = unique(wastinc_624_unadj[[i]]$meanLevel))
  print(Anywast_plot)
  REpooled_wastinc624_unadj<-rbind(REpooled_wastinc624_unadj, Anywast[Anywast$study=="Pooled estimate",])
  }
}
...
save(REpooled_wastinc624_unadj, file="C:/Users/andre/Documents/HBGDki/Results/REpooled_wastinc624_unadj.Rdata")

#Drop variables that don't make sense to pool
REpooled_wastinc624_unadj <- REpooled_wastinc624_unadj %>% filter(variable!="month" & variable!="birthmonth")

#filter out birthweight
REpooled_wastinc624_unadj <- REpooled_wastinc624_unadj %>% filter(variable!="BIRTHWT")


REpooled_wastinc624_unadj$reflabel <- ""
REpooled_wastinc624_unadj$reflabel[is.na(REpooled_wastinc624_unadj$RR)] <- "(ref.)"
REpooled_wastinc624_unadj$RR[is.na(REpooled_wastinc624_unadj$RR)] <- 1


#FIX to offset (ref.) by a relative amount
REpooled_wastinc624_unadj <- REpooled_wastinc624_unadj %>% group_by(variable) %>% mutate(RRrange=diff(range(RR), na.rm=T))


p<-ggplot(REpooled_wastinc624_unadj, aes(x=level)) + 
...
ggsave("Unadjusted Risk Factor Plots Wasting CI Pooled 6-24.pdf", p, height=10, width=10)
