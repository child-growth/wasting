...
load("C:/Users/andre/Documents/HBGDki/Results/Risk_Factor_Results_wastinc_Birthweight.Rdata")
...
Anywast624mo_plot <-RRplot_fun(Anywast624mo, 
                             reflevel="3000-3400 grams", 
                             title="Cumulative incidence ratios:\nAny wasting from 7-24 months age", 
                             units="",
                             levels = c("<=2700 grams","2700-3000 grams","3000-3400 grams",">3400 grams"))
Anywast624mo_plot
...
Anywast024mo_adj_plot <-RRplot_fun(Anywast024mo, 
                             reflevel="3000-3400 grams", 
                             title="Cumulative incidence ratios:\nAny wasting from 1-24 months age - Adjusted", 
                             units="",
                             levels = c("<=2700 grams","2700-3000 grams","3000-3400 grams",">3400 grams"))
...
wastrec60d_024mo_plot <-RRplot_fun(wastrec60d_024mo, 
                             reflevel="3000-3400 grams", 
                             title="Cumulative incidence ratios: Any recovery\nfrom wasting with 60 days (1-24 months age)", 
                             units="",
                             levels = c("<=2700 grams","2700-3000 grams","3000-3400 grams",">3400 grams"))
wastrec60d_024mo_plot


wastrec60d_624mo <- cleandf(wastrec_624_unadj$BIRTHWT, RF_levels=c("<=2700","2700-3000","3000-3400",">3400"))
wastrec60d_624mo_plot <-RRplot_fun(wastrec60d_624mo, 
                             reflevel="3000-3400 grams", 
                             title="Cumulative incidence ratios: Any recovery\nfrom wasting with 60 days (7-24 months age)", 
                             units="",
                             levels = c("<=2700 grams","2700-3000 grams","3000-3400 grams",">3400 grams"))
wastrec60d_624mo_plot
...
pdf("birthweightRF_plots.pdf",width=8,height=8)
...
# #Save plot DF's for sharing on OSF
# #Clean results data-frames down to necessary variables for sharing
# Anywast024mo <- Anywast024mo %>% subset(., select=-c(maxRR, order, x))
# Anywast624mo <- Anywast624mo %>% subset(., select=-c(maxRR, order, x))
# Anywast6mo <- Anywast6mo %>% subset(., select=-c(maxRR, order, x))
# wastrec60d <- wastrec60d %>% subset(., select=-c(maxRR, order, x))
# wastrec60d_024mo <- wastrec60d_024mo %>% subset(., select=-c(maxRR, order, x))
# wastrec60d_624mo <- wastrec60d_624mo %>% subset(., select=-c(maxRR, order, x))
# 
# Anywast024mo$reference <- ifelse(Anywast024mo$level=="3000-3400",1,0) 
# Anywast624mo$reference <- ifelse(Anywast624mo$level=="3000-3400",1,0) 
# Anywast6mo$reference <- ifelse(Anywast6mo$level=="3000-3400",1,0) 
# wastrec60d$reference <- ifelse(wastrec60d$level=="3000-3400",1,0) 
# wastrec60d_024mo$reference <- ifelse(wastrec60d_024mo$level=="3000-3400",1,0) 
# wastrec60d_624mo$reference <- ifelse(wastrec60d_624mo$level=="3000-3400",1,0) 
# 
# save(Anywast6mo,wastrec60d,Anywast624mo,wastrec60d_624mo,Anywast024mo,wastrec60d_024mo, file="BIRTHWT_IRres_plot_format.Rdata")
