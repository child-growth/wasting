...
wastinc_06_unadj <- wastinc_06_adj <-  list()
...
for(i in 1:length(factor_vars)){
  ...
  # wastinc_06_adj[[paste0(Avar)]] <- dsub %>% group_by(STUDYID, COUNTRY) %>% select_groups() %>%
  #   do(try(as.data.frame(  tmle_risk(dat=as.data.frame(.),
  #                                        Y="wast_inc",
...
save(wastinc_06_unadj, wastinc_06_adj, 
     file="U:/results/Risk_Factor_Results_wastinc0-6.Rdata")
