...
adjusted_lib <- c("SL.mean","SL.glm","bayesglm","SL.glmnet", "SL.gam")
...
wastinc_06_unadj <- wastinc_624_unadj <- wastinc_024_unadj <- wastrec_024_unadj <- wastinc_06_adj <- wastinc_624_adj <- wastinc_024_adj <- list()
wastrec_624_unadj <- list()
...
for(i in 1:length(continious_vars)){
  ...
  if(overall_dist==T){
    Acuts=quantile(d[,Avar], probs = c(.25,.5,.75), na.rm=T)
  ...
  wastinc_024_unadj[[paste0(Avar)]] <- dsub %>% group_by(STUDYID, COUNTRY) %>% select_groups() %>%
    do(try(as.data.frame(  tmle_risk(dat=as.data.frame(.),
                                         ...
                                         n.cat=4,
                                         A=Avar,
                                         Acuts=c(2700,3000,3400),
                                         Alevels=c("<=2700","2700-3000","3000-3400",">3400"),
                                         reflevel=3,
                                         family="binomial",
  ...
  wastinc_024_adj[[paste0(Avar)]] <- dsub %>% group_by(STUDYID, COUNTRY) %>% select_groups() %>%
    do(est=try(as.data.frame(  tmle_risk(dat=as.data.frame(.),
                                         Y="wast_inc",
                                         W=colnames(dsub)[which(!(colnames(dsub) %in% c("STUDYID","COUNTRY","SUBJID","AGEDAYS","HAZ","stunt","sevstunt","tr", Avar, paste0("miss_",Avar))))],
                                         n.cat=4,
                                         A=Avar,
                                         Acuts=c(2700,3000,3400),
                                         Alevels=c("<=2700","2700-3000","3000-3400",">3400"),
...
for(i in 1:length(continious_vars)){
  ...
  wastinc_06_unadj[[paste0(Avar)]] <- dsub %>% group_by(STUDYID, COUNTRY) %>% select_groups() %>%
    do(try(as.data.frame(  tmle_risk(dat=as.data.frame(.),
                                     Y="wast_inc",
                                     W=c("w1","w2"),
                                     n.cat=4,
...
for(i in 1:length(continious_vars)){
  ...
  wastinc_624_unadj[[paste0(Avar)]] <- dsub %>% group_by(STUDYID, COUNTRY) %>% select_groups() %>%
    do(try(as.data.frame(  tmle_risk(dat=as.data.frame(.),
                                     Y="wast_inc",
                                     W=c("w1","w2"),
                                     n.cat=4,
...
for(i in 1:length(continious_vars)){
  ...
  wastrec_024_unadj[[paste0(Avar)]] <- dsub %>% group_by(STUDYID, COUNTRY) %>% select_groups() %>%
    do(try(as.data.frame(  tmle_risk(dat=as.data.frame(.),
                                     Y="wast_rec60d",
                                     W=c("w1","w2"),
                                     n.cat=4,
...
load("U:/data/Compiled Datasets/WastRec6-24.Rdata")
d <- d %>% filter(is.na(tr) | tr=="C" | miss_tr==1) %>% as.data.frame()
d$COUNTRY <- as.character(d$COUNTRY)
d$STUDYID <- as.character(d$STUDYID)
...
for(i in 1:length(continious_vars)){
  ...
  wastrec_624_unadj[[paste0(Avar)]] <- dsub %>% group_by(STUDYID, COUNTRY) %>% select_groups() %>%
    do(try(as.data.frame(  tmle_risk(dat=as.data.frame(.),
                                     Y="wast_rec60d",
                                     W=c("w1","w2"),
                                     n.cat=4,
  ...
  # wastinc_624_adj[[paste0(Avar)]] <- dsub %>% group_by(STUDYID, COUNTRY) %>% select_groups() %>%
  #   do(est=try(as.data.frame(  tmle_risk(dat=as.data.frame(.),
  #                                        Y="wast_inc",
  #                                        W=colnames(dsub)[which(!(colnames(dsub) %in% c("STUDYID","COUNTRY","SUBJID","AGEDAYS","HAZ","stunt","sevstunt","tr", Avar, paste0("miss_",Avar))))],
  #                                        n.cat=4,
...
save(wastinc_06_unadj, wastinc_624_unadj,
     wastinc_024_unadj, wastinc_024_adj, wastrec_024_unadj, wastrec_624_unadj,
     file="U:/results/Risk_Factor_Results_wastinc_Birthweight.Rdata")
