...
load("U:/data/Compiled Datasets/WastInc0-6_noBW.Rdata")
d <- d %>% filter(is.na(tr) | tr=="C" | miss_tr==1) %>% as.data.frame()
d$COUNTRY <- as.character(d$COUNTRY)
d$STUDYID <- as.character(d$STUDYID)
...
load("U:/data/Compiled Datasets/WastInc6-24_noBW.Rdata")
d <- d %>% filter(is.na(tr) | tr=="C" | miss_tr==1) %>% as.data.frame()
d$COUNTRY <- as.character(d$COUNTRY)
d$STUDYID <- as.character(d$STUDYID)
...
save(wastinc_06_unadj, wastinc_624_unadj,
     wastinc_024_unadj, wastinc_024_adj,
     file="U:/results/Risk_Factor_Results_wastinc_Birthweight_noBirthInc.Rdata")
