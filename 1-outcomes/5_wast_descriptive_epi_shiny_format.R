...
sev.prev.India <- summary.prev(d[d$country=="INDIA",], severe.wasted = T)$prev.res
sev.prev.cohort <-
  sev.prev.data$prev.cohort %>% subset(., select = c(cohort, region, agecat, nmeas,  prev,  ci.lb,  ci.ub)) %>%
  rename(est = prev,  lb = ci.lb,  ub = ci.ub)

sev.prev <- bind_rows(
  data.frame(cohort = "pooled", region = "India", sev.prev.India),
...
whz.India <- summary.whz(d[d$country=="INDIA",])$whz.res
whz.cohort <-
...
whz <- bind_rows(
  ...
  data.frame(cohort = "pooled", region = "India", whz.India),
...
ci.India <- summary.ci(d[d$country=="INDIA",], agelist = agelst)$ci.res
ci.cohort <-
...
ci <- bind_rows(
  ...
  data.frame(cohort = "pooled", region = "India", ci.India),
...
ci.India3 <- summary.ci(d3[d3$country=="INDIA",], agelist = agelst3)$ci.res
ci.cohort3 <-
...
ci_3 <- bind_rows(
  ...
  data.frame(cohort = "pooled", region = "India", ci.India3),
...
ci.India.nobirth <- summary.ci(d_noBW[d_noBW$country=="INDIA",], agelist = agelst)$ci.res
ci.cohort.nobirth <-
...
ci_nobw <- bind_rows(
  ...
  data.frame(cohort = "pooled", region = "India", ci.India.nobirth),
...
ci.India.nobirth3 <- summary.ci(d3[d3$country=="INDIA",], agelist = agelst3)$ci.res
ci.cohort.nobirth3 <-
...
ci_nobw3 <- bind_rows(
  ...
  data.frame(cohort = "pooled", region = "India", ci.India.nobirth3),
...
sev.ci.India <- summary.ci(d[d$country=="INDIA",], agelist = agelst, severe.wasted = T)$ci.res
...
sev.ci <- bind_rows(
  ...
  data.frame(cohort = "pooled", region = "India", sev.ci.India),
...
ir.India <- summary.ir(d[d$country=="INDIA",], agelist = agelst)$ir.res
ir.cohort <-
...
ir <- bind_rows(
  ...
  data.frame(cohort = "pooled", region = "India", ir.India),
...
ir.India.nobirth <- summary.ir(d_noBW[d_noBW$country=="INDIA",], agelist = agelst)$ir.res
ir.cohort.nobirth <-
  ir.data$ir.cohort %>% subset(., select = c(cohort, region, agecat,  yi,  ci.lb,  ci.ub)) %>%
  rename(est = yi,  lb = ci.lb,  ub = ci.ub)
...
ir_noBW <- bind_rows(
  data.frame(cohort = "pooled", region = "Overall", ir.data$ir.res),
  data.frame(cohort = "pooled", region = "India", ir.India.nobirth),
  data.frame(cohort = "pooled", ir.region),
  ci.cohort
)
...
sev.ir.India <- summary.ir(d[d$country=="INDIA",], agelist = agelst, sev.wasting = T)$ir.res
sev.ir.cohort <-
...
rec.data30.India <- summary.rec60( d[d$country=="INDIA",], length = 30, agelist = c("0-6 months", "6-12 months", "12-18 months", "18-24 months"))$ci.res
rec.data60.India <- summary.rec60( d[d$country=="INDIA",], length = 60, agelist = c("0-6 months", "6-12 months", "12-18 months", "18-24 months"))$ci.res
rec.data90.India <- summary.rec60( d[d$country=="INDIA",], length = 90, agelist = c("0-6 months", "6-12 months", "12-18 months", "18-24 months"))$ci.res
...
perswast.India <- summary.perswast(d[d$country=="INDIA",], agelist = c("0-6 months", "6-12 months", "12-18 months", "18-24 months"))$pers.res
perswast.cohort <-
...
perswast <- bind_rows(
  ...
  data.frame(cohort = "pooled", region = "India", perswast.India),
...
sev.ci.India3 <- summary.ci(d3[d3$country=="INDIA",], agelist = agelst3, severe.wasted = T)$ci.res
sev.ci.cohort <-
  sev.ci.data$ci.cohort %>% subset(., select = c(cohort, region, agecat,  yi,  ci.lb,  ci.ub)) %>%
  rename(est = yi,  lb = ci.lb,  ub = ci.ub)
...
sev.ci_noRec <- bind_rows(
  data.frame(cohort = "pooled", region = "Overall", sev.ci.data$ci.res),
  data.frame(cohort = "pooled", sev.ci.region),
  data.frame(cohort = "pooled", region = "India", sev.ci.India3),
  sev.ci.cohort
)
...
ir.India <- summary.ir(d_noRec[d_noRec$country=="INDIA",], agelist = agelst)$ir.res
ir.cohort <-
  ir.data$ir.cohort %>% subset(., select = c(cohort, region, agecat,  yi,  ci.lb,  ci.ub)) %>%
  rename(est = yi,  lb = ci.lb,  ub = ci.ub)
...
shiny_desc_data <- bind_rows(
  data.frame(outcome = "Wasting prevalence", prev),
  data.frame(outcome = "Severe wasting prevalence", sev.prev),
  data.frame(outcome = "Mean WHZ",  whz),
  data.frame(outcome = "Cumulative incidence", ci),
  data.frame(outcome = "Cumulative incidence - 3 month range", ci_3),
  data.frame(outcome = "Cumulative incidence - no birth wasting",  ci_nobw),
  data.frame(outcome = "Cumulative incidence - no birth wasting, 3 month range",  ci_nobw3),
  data.frame(outcome = "Cumulative incidence - severe wasting",  sev.ci),
  data.frame(outcome = "Incidence rate", ir),
  data.frame(outcome = "Incidence rate - no birth wasting", ir_noBW),
  data.frame(outcome = "Incidence rate - severe wasting", sev.ir),
  data.frame(outcome = "Persistent wasting",  perswast),
  data.frame(outcome = "Recovery-30d", rec30),
  data.frame(outcome = "Recovery-60d", rec60),
  data.frame(outcome = "Recovery-90d", rec90),
  data.frame(outcome = "Wasting onset", ci_3_noRec),
  data.frame(outcome = "Severe wasting onset", sev.ci_noRec),
  data.frame(outcome = "Wasting onset rate", ir_noRec)
  )
...
shiny_desc_data$region <- factor(shiny_desc_data$region, levels=c("Overall", "Africa", "Latin America", "Asia", "India"))
