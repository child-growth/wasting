...
d <- d %>% group_by(studyid, country, subjid) %>% mutate(born_wast = 1*(first(whz) < (-2)))
table(d$born_wast)

d <- d %>% filter(born_wast==1) %>% filter(agedays > 30)
...
save(shiny_desc_data, file = "U:/Data/Wasting/desc_data_bornWast.Rdata")
