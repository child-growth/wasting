...
#Plot parameters
textcol <- "grey20"
plottitle="Cumulative incidence of wasting"
...
hm2 <- hm + aes(fill=RRcat) +
  labs(x="Exposure level",y="",title=plottitle) + theme(legend.position="none") +
  scale_fill_viridis_d(na.value="grey80", drop=F) + 
  labs(fill="Relative Risk")  + guides(fill = guide_legend(reverse=T))
hm2

setwd("C:/Users/andre/Dropbox/HBGDki figures/Berlin wasting figures")
ggsave(hm2, file="Wasting_RF_heatmap.png", width=9.2, height=5.6)
...
hm3 <- ggplot(RMAestfull,aes(x=order,y=RFlabel)) +
  ...
  scale_fill_viridis_d(na.value="grey80", drop=F) + labs(fill="Relative Risk") #+ geom_text(aes(label=paste0(intervention_level,"\nRR:",round(RR2,2))))
hm3

ggsave(hm3, file="Wasting_RF_heatmap_greyNA.png", width=9.2, height=5.6)
...
ggsave(hm4, file="Wasting_RF_heatmap_justsig.png", width=9.2, height=5.6)
...
ggsave(hm5, file="Wasting_RF_heatmap_justsig_lable.png", width=9.2, height=5.6)
