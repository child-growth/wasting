...
p1 <- ggplot(prev.res,aes(y=est,x=agecat)) +
  ...
  annotate("text",x=prev.res$agecat,y=1,label=prev.res$nstudy.f,size=3)+
...
p3 <- ggplot(rev.res,aes(y=est,x=agecat))+
  ...
  annotate("text",x=rev.res$agecat,y=21,label=rev.res$nmeas.f,size=3)+
  annotate("text",x=rev.res$agecat,y=25,label=rev.res$nstudy.f,size=3)+
  annotate("text",label=rev.res$ptest.f,x=rev.res$agecat,
...
p4_nobirth <- ggplot(df, aes(y=est*1000,x=agecat.f2, fill=agecat.f2, color=agecat.f2))+
  ...
  annotate("text",x=df$agecat.f2,y=-0.25,label=df$nmeas.f,size=3) +
  annotate("text",x=df$agecat.f2,y=0,label=df$nstudy.f,size=3) +
  annotate("text",label=df$ptest.f,x=df$agecat.f2, y=df$est*1000,hjust=-1,size=3)+
  theme(strip.background = element_blank(), strip.text.x = element_text(size=12)) +
  scale_colour_manual(values=rep(tableau10[2],20)) +
