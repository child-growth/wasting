...
#Subset to adjusted
d <- d %>% filter(adjustment_set!="unadjusted" | outcome_variable=="sex")
...
d <- d %>% filter(agecat=="0-6 months"| agecat=="6 months"| agecat=="6-24 months"| agecat=="24 months"| agecat=="0-24 months")
...
d <- d %>% filter(outcome_variable=="wast_rec90d")
...
setwd("C:/Users/andre/Dropbox/HBGDki figures/Risk Factor Analysis/Wasting/Recovery pooled/")
pdf("Risk Factor Plots Wasting Recovery", height=8, width=12)
...
for(i in levels(RMAest$intervention_variable)){
  ...
  if(nrow(plotdf)>0){
    ...
    ggsave(p, file=paste0("Recovery pooled",j,"_",i,"_RFplot.png"), width=5.7, height=4.6)
